...
export function TelemetryDevicePage(){
 ...
 const columns=useMemo<DataTableColumn<TelemetryDeviceRow>[]>(()=>[
  ...
  {key:"vehicleStatus",label:"Vehicle Status",value:r=>r.vehicleStatus,render:r=><VehicleStatus status={r.vehicleStatus}/>},
...
function VehicleStatus({status}:{status:string}){const normalized=status.toUpperCase();return <StatusIndicator status={normalized} label={["STOP","IDLE","TRIP"].includes(normalized)?normalized:"STOP"}/>;}
