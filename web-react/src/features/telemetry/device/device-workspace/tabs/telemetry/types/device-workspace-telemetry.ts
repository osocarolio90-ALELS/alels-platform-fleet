...
export type WorkspaceTrackPoint = { latitude:number; longitude:number; angle?:number|null; speed?:number|null; occurredAt:string };
export type WorkspaceConnection = { signalStrength?:number|null; satellitesUsed?:number|null; gnssStatus:string; tcpStatus:string; protocol?:string|null; channel?:string|null };
export type WorkspacePacket = { id:number; sequence?:number|null; receivedAt:string };
export type WorkspaceDataParameter = { fieldCode:string; label:string; value?:string|null; numericValue?:number|null; booleanValue?:boolean|null; unit?:string|null; parameterId?:string|null; category?:string|null; sourceProtocol?:string|null; dictionaryCode?:string|null; deviceModelId?:number|null };
...
export type WorkspaceTelemetry = { device:WorkspaceDevice; driver:WorkspaceDriver; vehicle:WorkspaceVehicle; position:WorkspacePosition; connection:WorkspaceConnection; packet?:WorkspacePacket|null; vehicleStatus?:string|null; dataReceived:WorkspaceDataParameter[]; parameterGroups:string[]; configuration:WorkspaceConfiguration; track:WorkspaceTrackPoint[] };
