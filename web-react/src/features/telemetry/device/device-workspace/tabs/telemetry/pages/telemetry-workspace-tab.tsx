...
function Status({value}:{value?:string|null}){const normalized=(value||"UNKNOWN").toUpperCase();const tone=/(ONLINE|ACTIVE|OPERATIONAL|ON.DUTY|VALID|RESOLVED)/.test(normalized)?"success":/(WARNING|IDLE|MAINTENANCE)/.test(normalized)?"warning":/(CRITICAL|DANGER|ERROR|OFFLINE|SUSPENDED)/.test(normalized)?"danger":"muted";return <Badge tone={tone}>{normalized}</Badge>;}
