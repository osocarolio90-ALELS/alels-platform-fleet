...
export function DeviceWorkspacePage(){
 ...
 return <div className={cn("legacy-shell min-h-screen p-5",theme==="light"?"theme-light":"theme-dark")}><div ref={workspaceRef} className="legacy-content dw-workspace">
  <DeviceWorkspaceHeader imei={device?.imei} model={device?`${device.brand} ${device.model}`:"Loading device..."} online={Boolean(device?.online)} vehiclePlate={vehiclePlate} location={location} actions={<>
  ...
  <nav className="dw-tabs" aria-label="Device workspace tabs"><button type="button" className="active">Telemetry</button><button type="button" disabled>Trip &amp; Route</button><button type="button" disabled>Log &amp; Message</button><button type="button" disabled>Command</button><button type="button" disabled>Analytics</button></nav>
  <TelemetryWorkspaceTab deviceId={deviceId} live={live} refreshToken={refreshToken} clusterRef={clusterRef}/>
 </div></div>;
}
