...
import { ArrowRightLeft, Eye, IdCard, Pencil, Plus, Save, Trash2, X } from "lucide-react";
...
export function DriverRegisterPage() {
  ...
  const [moveRows, setMoveRows] = useState<DriverRegisterRow[]>([]);
  ...
  const moveMutation = useMutation({
    mutationFn: (targetCompanyId: number) => moveAssets("DRIVER", moveRows.map((row) => row.id), targetCompanyId),
    onSuccess: () => { setMoveRows([]); setNotice("Driver berhasil dipindahkan."); queryClient.invalidateQueries({ queryKey: ["driver-register"] }); },
    onError: (error) => setNotice(error instanceof Error ? error.message : "Driver gagal dipindahkan.")
  });
  ...
  const bulkActions = useMemo<DataTableBulkAction<DriverRegisterRow>[]>(() => [
    ...(canMove ? [{ key: "move", label: "Move", icon: <ArrowRightLeft className="h-4 w-4" />, onClick: async (rows: DriverRegisterRow[]) => setMoveRows(rows) }] : []),
