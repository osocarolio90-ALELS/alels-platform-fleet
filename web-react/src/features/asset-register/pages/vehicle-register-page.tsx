      ...
      <section className="space-y-5 text-foreground">
        ...
        <OrganizationTableCard>
          ...
          <form className="grid gap-4 md:grid-cols-2" onSubmit={submit}>
            ...
            <Field label="Plat Number"><Input required value={form.plateNumber} onChange={(e) => setForm((c) => ({ ...c, plateNumber: e.target.value.toUpperCase() }))} placeholder="B 1234 ABC" /></Field>
...
function fromRow(row: VehicleRegisterRow): VehicleForm { return { companyId: String(row.companyId), vehicleName: row.vehicleName || "", plateNumber: row.plateNumber || "", vehicleTypeId: row.vehicleTypeId ? String(row.vehicleTypeId) : "", brandId: row.brandId ? String(row.brandId) : "", modelId: row.modelId ? String(row.modelId) : "", yearManufacture: row.yearManufacture ? String(row.yearManufacture) : String(currentYear), countryCode: row.countryCode || "ID", energyCode: row.energyCode || "", ownershipTypeId: row.ownershipTypeId ? String(row.ownershipTypeId) : "", capacityValue: row.capacityValue ? String(row.capacityValue) : "", capacityUnitId: row.capacityUnitId ? String(row.capacityUnitId) : "", notes: "" }; }
function normalizeInput(form: VehicleForm): VehicleRegisterInput { return { companyId: form.companyId ? Number(form.companyId) : null, vehicleCode: null, vehicleName: form.vehicleName.trim() || null, plateNumber: form.plateNumber.trim(), vehicleTypeId: form.vehicleTypeId ? Number(form.vehicleTypeId) : null, brandId: form.brandId ? Number(form.brandId) : null, modelId: form.modelId ? Number(form.modelId) : null, yearManufacture: form.yearManufacture ? Number(form.yearManufacture) : null, countryCode: form.countryCode || "ID", energyCode: form.energyCode || null, ownershipTypeId: form.ownershipTypeId ? Number(form.ownershipTypeId) : null, capacityValue: form.capacityValue ? Number(form.capacityValue) : null, capacityUnitId: form.capacityUnitId ? Number(form.capacityUnitId) : null, operationalStatus: "UNKNOWN", notes: form.notes.trim() || null }; }
