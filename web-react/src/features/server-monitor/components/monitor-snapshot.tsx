        ...
        {items.map((item) => (
          <div key={item.key} className="flex items-center justify-between gap-3 rounded-xl bg-muted px-3 py-3 ring-1 ring-border">
            <div className="flex min-w-0 items-center gap-2">
              <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-emerald-50 text-emerald-800">
                {item.icon ?? fallbackIcon}
              </span>
              <span className="truncate text-xs font-extrabold text-card-foreground">{item.label}</span>
            </div>
